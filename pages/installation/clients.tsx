...
import { Grid, Link, Button, Chip, CircularProgress, Box, Avatar } from '@mui/material';
...
const ClientsList = () => {
    ...
    const columns: GridColDef[] = [
        ...
        { field: 'company', headerName: 'Compañia', width: 300,  renderCell: ({row}: GridValueGetterParams) => {
            return (
                <NextLink  passHref href={`/installation/client-${row.id}`}  >
...
useEffect(()=>{
  ...
  return (
    ...
    <Box className="card">
    <CardHeader1  
                        title="Seleccione una compañia para ver luego registrar un ticket" 
                        avatar={ 
            ...
            <Grid item xs={12} md={8} sx={{ padding: "30px 0", width: '100%' }}>
            ...
            <Grid item xs={12} md={4} sx={{ padding: "30px 0", width: '100%' }}>
                <Link
                    onClick={ () =>  router.push('/installation/client-6299e33b05e471715ccb9f5e')}>
                    <Button   color="secondary"  className="circular-btn buttonSubmit" variant="outlined" >Registrar nuevo sin cliente</Button> 
                </Link>
            </Grid>            
