...
import { taskByUserLoadGrid, taskTotalByStatusByUser } from '../../handlers/task'
...
const ListsTaskPage = () => {
    ...
    const initialData = async ( userId ) => {
        setLoading(true);
        try{
            const { tasks, okTasks } = await taskByUserLoadGrid({ filter: { userId } });  
            if(okTasks) {           
...
const openTask = async(param) => {
    setLoading(true);
    try{
    const data1 = await taskTotalByStatusByUser(param);
  ...
  return (
    ...
    >
        ...
        <Box className="card">
        <Typography className='Maintitle'>Mis Tareas</Typography>
